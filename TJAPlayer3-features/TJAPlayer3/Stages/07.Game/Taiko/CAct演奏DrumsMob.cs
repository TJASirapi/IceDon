        ...
        public override int On進行描画()
        {
            if(!TJAPlayer3.stage演奏ドラム画面.bDoublePlay)
            {
                ...
                if(TJAPlayer3.Skin.Game_Mob_Ptn != 0 && TJAPlayer3.ConfigIni.eGaugeMode != EGaugeMode.Hard && TJAPlayer3.ConfigIni.eGaugeMode != EGaugeMode.ExHard)
                {
                    ...
                    {
                        TJAPlayer3.Tx.Mob[(int)ctMobPtn.db現在の値]?.t2D描画(TJAPlayer3.app.Device, 0, (1080 - (TJAPlayer3.Tx.Mob[0].szテクスチャサイズ.Height - 70)) + -((float)Math.Sin((float)this.ctMob.db現在の値 * (Math.PI / 180)) * 70));
                    }
