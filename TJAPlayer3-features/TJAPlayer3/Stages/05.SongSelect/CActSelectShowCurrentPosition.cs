	...
	internal class CActSelectShowCurrentPosition : CActivity
	{
		...
		public override int On進行描画()
		{
			if ( this.txScrollBar != null )
			{
                ...
                //this.txScrollBar.t2D描画( CDTXMania.app.Device, (int)(1920- ((429.0f / 100.0f ) * CDTXMania.stage選曲.ct登場時アニメ用共通.n現在の値)), 164, new Rectangle( 0, 0, 352, 26 ) ); //移動後のxは851
			    #endregion
			    #region [ スクロール地点の描画 (計算はCActSelect曲リストで行う。スクロール位置と選曲項目の同期のため。)#27648 ]
				int py = TJAPlayer3.stage選曲.nスクロールバー相対y座標;
				if( py <= 336 && py >= 0 )
				{
					//this.txScrollBar.t2D描画( CDTXMania.app.Device, (int)( 1920 - 4 - (( 424.0f / 100.0f ) * CDTXMania.stage選曲.ct登場時アニメ用共通.n現在の値 ) ) + py, 164, new Rectangle( 352, 0, 26, 26 ) );//856
				}
			    #endregion
            }
