		...
		public void t全画面_ウィンドウモード切り替え()
		{
...
#endif
			{
				...
				if (ConfigIni.bウィンドウモード == true)    // #23510 2010.10.27 yyagi: to resume window size from backuped value
				{
					...
					base.Window.Icon = IceDon.Properties.Resources.RainDon;
...
for (int i = 0; i < 3; i++) {
	stage演奏ドラム画面.fDamageLevelFactor[i] = ConfigIni.fDamageLevelFactor[i];
		...
		private void t起動処理()
		{
			...
			base.Window.Icon = global::IceDon.Properties.Resources.RainDon;
			...
			#region Discordの処理
			Discord.Initialize("732155113220603914");
			StartupTime = Discord.GetUnixTime();
			Discord.UpdatePresence("", IceDon.Properties.Discord.Stage_StartUp, StartupTime);
			#endregion
